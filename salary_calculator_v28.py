...
class SalaryCalculatorApp:
    def __init__(self, root):
        ...
        self.data = {
            ...
            "nation_commission_rate": tk.StringVar(value="0.1"),
            "region_commission_rate": tk.StringVar(value="0.4"),
            "has_region": tk.BooleanVar(value=True),
            "staff_data": []
        }
    ...
    def create_widgets(self):
        ...
        # 旧月均提成计算器tab页
        old_commission_frame = ttk.Frame(notebook)
        notebook.add(old_commission_frame, text="旧月均提成计算器")
        self.create_old_commission_tab(old_commission_frame)
    ...
    def create_old_commission_tab(self, parent):
        paramf = ttk.LabelFrame(parent, text="参数填写")
        paramf.pack(fill=tk.X, padx=10, pady=10)
        self.old_commission_vars = {
            "delivery_volume": tk.StringVar(value="175"),
            "service_packs": tk.StringVar(value="102"),
            "store_staff": tk.StringVar(value="2"),
            "pack_price": tk.StringVar(value="510"),
        ...
        row = 0
        ttk.Label(paramf, text="交付量").grid(row=row, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(paramf, textvariable=self.old_commission_vars["delivery_volume"], width=10).grid(row=row, column=1)
        ttk.Label(paramf, text="购买服务包数").grid(row=row, column=2, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(paramf, textvariable=self.old_commission_vars["service_packs"], width=10).grid(row=row, column=3)
        row += 1
        ttk.Label(paramf, text="门店人数").grid(row=row, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(paramf, textvariable=self.old_commission_vars["store_staff"], width=10).grid(row=row, column=1)
        ttk.Label(paramf, text="服务包价格").grid(row=row, column=2, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(paramf, textvariable=self.old_commission_vars["pack_price"], width=10).grid(row=row, column=3)
        row += 1
        ttk.Label(paramf, text="是否站内上牌").grid(row=row, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Combobox(paramf, textvariable=self.old_commission_vars["has_station_license"], width=8,
                     values=["有", "无"]).grid(row=row, column=1)
        ttk.Label(paramf, text="全国总提成比例").grid(row=row, column=2, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(paramf, textvariable=self.old_commission_vars["nation_commission_rate"], width=10).grid(row=row, column=3)
        row += 1
        ttk.Label(paramf, text="区总提成比例").grid(row=row, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(paramf, textvariable=self.old_commission_vars["region_commission_rate"], width=10).grid(row=row, column=1)
        ttk.Checkbutton(paramf, text="有区总", variable=self.old_commission_vars["has_region"]).grid(row=row, column=2, sticky=tk.W, padx=5, pady=2)
        row += 1
        calc_btn = ttk.Button(paramf, text="计算旧月均提成", command=self.calculate_old_commission)
        calc_btn.grid(row=row, column=0, columnspan=2, pady=8)
        self.old_commission_to_main_btn = ttk.Button(paramf, text="一键填入主页面", command=self.fill_to_main, state="disabled")
        self.old_commission_to_main_btn.grid(row=row, column=2, columnspan=2, pady=8)

        self.old_commission_result = tk.Text(parent, height=10, wrap=tk.WORD, font=("Consolas", 10))
        self.old_commission_result.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

    def calculate_old_commission(self):
        try:
            delivery_volume = int(self.old_commission_vars["delivery_volume"].get())
            service_packs = int(self.old_commission_vars["service_packs"].get())
            store_staff = int(self.old_commission_vars["store_staff"].get())
            pack_price = int(self.old_commission_vars["pack_price"].get())
            has_station_license = self.old_commission_vars["has_station_license"].get() == "有"
            nation_commission_rate = float(self.old_commission_vars["nation_commission_rate"].get())
            region_commission_rate = float(self.old_commission_vars["region_commission_rate"].get())
            has_region = self.old_commission_vars["has_region"].get()

            conversion_rate = (service_packs / delivery_volume) * 100 if delivery_volume > 0 else 0

            if pack_price > 450 and not has_station_license:
                if conversion_rate < 50:
                    ...
                    commission_rate = 20
            elif pack_price > 450 and has_station_license:
                if conversion_rate < 50:
                    commission_rate = 15
                elif 50 <= conversion_rate < 60:
                    commission_rate = 20
                    ...
                    commission_rate = 25
            elif pack_price <= 450 and not has_station_license:
                if conversion_rate < 50:
                    commission_rate = 5
                elif 50 <= conversion_rate < 60:
                    commission_rate = 10
            ...
            total_bonus = service_packs * commission_rate
            nation_amount = total_bonus * nation_commission_rate
            if has_region:
                region_amount = (total_bonus - nation_amount) * region_commission_rate
                staff_amount = (total_bonus - nation_amount) * (1 - region_commission_rate)
            else:
                region_amount = 0
                staff_amount = total_bonus - nation_amount

            avg_staff_commission = staff_amount / store_staff if store_staff > 0 else 0

            text = ""
            text += f"1. 转化率 = 购买服务包数 / 交付量 × 100% = {service_packs} / {delivery_volume} × 100% = {conversion_rate:.2f}%\n"
            text += f"2. 单包提成金额 = {commission_rate} 元/包\n"
            text += f"3. 门店总奖金 = 购买服务包数 × 单包提成金额 = {service_packs} × {commission_rate} = {total_bonus:.2f} 元\n"
            text += f"4. 全国总提成 = {total_bonus:.2f} × {nation_commission_rate*100:.1f}% = {nation_amount:.2f} 元\n"
            if has_region:
                text += f"5. 区总提成 = (总奖金-全国总提成) × {region_commission_rate*100:.1f}% = ({total_bonus:.2f}-{nation_amount:.2f})×{region_commission_rate*100:.1f}% = {region_amount:.2f} 元\n"
                text += f"6. 员工可分部分 = (总奖金-全国总提成) × {100-region_commission_rate*100:.1f}% = ({total_bonus:.2f}-{nation_amount:.2f})×{100-region_commission_rate*100:.1f}% = {staff_amount:.2f} 元\n"
            else:
                text += f"5. 员工可分部分 = 总奖金-全国总提成 = {total_bonus:.2f} - {nation_amount:.2f} = {staff_amount:.2f} 元\n"
            text += f"7. 门店每人提成 = 员工可分部分 / 门店人数 = {staff_amount:.2f} / {store_staff} = {avg_staff_commission:.2f} 元\n"
            text += f"\n【一键填回主参数页：旧月均提成 = {avg_staff_commission:.2f} 元/人】"
            self.old_commission_result.config(state=tk.NORMAL)
            self.old_commission_result.delete(1.0, tk.END)
            self.old_commission_result.insert(tk.END, text)
            self.old_commission_result.config(state=tk.DISABLED)
            self.old_commission_value = f"{avg_staff_commission:.2f}"
            self.old_commission_to_main_btn.config(state="normal")
        except Exception as e:
            messagebox.showerror("计算错误", f"旧月均提成计算错误：{str(e)}")
            self.old_commission_to_main_btn.config(state="disabled")

    def fill_to_main(self):
        self.data["old_avg_commission"].set(self.old_commission_value)
        messagebox.showinfo("已填入", f"已将旧月均提成 {self.old_commission_value} 元/人写入主参数页！")

    # 下面是其它 create_params_tab、create_formulas_tab、reset_data 方法不变...
    # ... 你的原有方法都继续放在类里 ...
    ...
    def create_params_tab(self, parent):
        ...
        for i, (label, key) in enumerate(threshold_labels):
            ...
            if key == "use_threshold":
                ...
                # 档1提成显示只读
                self.commission_tier1_entry = ttk.Entry(row_frame, textvariable=self.data[key], width=10,
                                                        state="disabled")
        ...
        # 在基础参数tab最后一行加“有区总”勾选框
        row = len(basic_labels)
        ttk.Label(basic_frame, text="有区总:").grid(row=row, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Checkbutton(basic_frame, variable=self.data["has_region"]).grid(row=row, column=1, padx=5, pady=2)
    ...
    def calculate(self):
        try:
            ...
            # 重新读取参数，保证联动
            nation_commission_rate = float(self.data["nation_commission_rate"].get())
            region_commission_rate = float(self.data["region_commission_rate"].get())
            has_region = self.data["has_region"].get()
            if isinstance(has_region, str):
                has_region = has_region.lower() in ("1", "true", "yes", "on")
            elif isinstance(has_region, int):
                has_region = bool(has_region)
            # 调试可留
            # print("调试：has_region（类型保险）为：", has_region)
            ...
            for i, staff in enumerate(staff_list):
                ...
                nation_amount = total_commission * nation_commission_rate
                if has_region:
                    region_amount = (total_commission - nation_amount) * region_commission_rate
                    staff_amount = (total_commission - nation_amount) * (1 - region_commission_rate)
                else:
                    region_amount = 0
                    staff_amount = total_commission - nation_amount
                per_capita_commission = staff_amount / store_staff if store_staff > 0 else 0
                ...
                result_text += f"  门店总奖金 = 购买服务包数 × 单包提成金额 = {service_packs} × {commission_rate} = {total_commission:.2f}元\n"
                result_text += f"  全国总提成 = 门店总奖金 × 全国总提成比例 = {total_commission:.2f} × {nation_commission_rate:.2f} = {nation_amount:.2f}元\n"
                if has_region:
                    result_text += f"  区总提成 = (门店总奖金 - 全国总提成) × 区总提成比例 = ({total_commission:.2f} - {nation_amount:.2f}) × {region_commission_rate:.2f} = {region_amount:.2f}元\n"
                    result_text += f"  员工可分部分 = (门店总奖金 - 全国总提成) × (1-区总提成比例) = ({total_commission:.2f} - {nation_amount:.2f}) × {1 - region_commission_rate:.2f} = {staff_amount:.2f}元\n"
                else:
                    result_text += f"  员工可分部分 = 门店总奖金 - 全国总提成 = {total_commission:.2f} - {nation_amount:.2f} = {staff_amount:.2f}元\n"
                result_text += f"  人均提成 = 员工可分部分 / 门店人数 = {staff_amount:.2f} / {store_staff} = {per_capita_commission:.2f}元\n"
            ...
            result_text += "\n模式二(新底薪中):\n"
            for i, staff in enumerate(staff_list):
                numerator = staff["old_base"] + staff["basic_perf"] + staff["post_perf"] + staff[
                    "subsidy"] + old_avg_commission - new_base_mid
                ...
                if use_threshold:
                    ...
                    if new_service_packs <= threshold1:
                        ...
                        total_commission = threshold1 * commission_tier1 + (
                                    new_service_packs - threshold1) * commission_tier2
                    else:
                        total_commission = threshold1 * commission_tier1 + (
                                    threshold2 - threshold1) * commission_tier2 + (
            ...
            for i, staff in enumerate(staff_list):
                numerator = staff["old_base"] + staff["basic_perf"] + staff["post_perf"] + staff[
                    "subsidy"] + old_avg_commission - new_base_low
                denominator = avg_monthly_packs
                if denominator == 0:
                    messagebox.showerror("错误", "过去X月平均每人购买服务包数量不能为零")
                    return
